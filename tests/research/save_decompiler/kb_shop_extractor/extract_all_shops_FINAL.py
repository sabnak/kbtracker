#!/usr/bin/env python3
"""
FINAL COMPLETE SHOP EXTRACTOR

Extracts ALL shops from save file with all 4 sections:
- Garrison (stored army)
- Items (equipment for sale)
- Units (troops for hire)
...
def find_all_shop_ids(data: bytes) -> list:
	...
	# Search through data for shop IDs
	pos = 0
	while pos < len(data):
		# Search for itext_m pattern in UTF-16 LE
		# Try to decode chunks and search
		chunk_size = 10000
...
def parse_entry_based(data: bytes, section_pos: int, next_pos: int) -> list:
	"""
	Parse entry-based format: length/name/qty/metadata/...
			...
			if 5 <= name_length <= 100:
				...
				try:
					...
					if is_valid_id(name):
						...
						if 0 < quantity < 10000:
							if name not in items_dict or items_dict[name] < quantity:
	...
	return sorted([(name, qty) for name, qty in items_dict.items()])
...
def parse_shop(data: bytes, shop_id: str, shop_pos: int) -> dict:
	...
	# Parse items (entry-based format)
	if items_pos:
	...
	# Parse spells (entry-based format)
	if spells_pos:
...
if __name__ == '__main__':
	...
	print("="*78)
	print("FINAL COMPLETE SHOP EXTRACTOR")
	print("="*78)
	print()
	...
	# Save to JSON
	output_file = os.path.join(script_dir, 'tmp', 'all_shops_FINAL.json')
	...
	print(f"\n[SUCCESS] Saved to: tmp/all_shops_FINAL.json")
